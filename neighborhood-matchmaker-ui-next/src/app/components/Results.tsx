...
import type { NeighborhoodSearchDTO, SearchResult } from '../types/search';
...
const Results = () => {
  ...
  if (!searchResults || searchResults.neighborhoods.length === 0) {
    return (
      <div className={styles.container}>
  ...
  const topThree = searchResults.neighborhoods.slice(0, 3);
  const remaining = searchResults.neighborhoods.slice(3);
        ...
        <div className={styles.criteriaGrid}>
          {searchCriteria?.budget && (
            <div className={styles.criteriaItem}>
              ...
              <span className={styles.criteriaValue}>${searchCriteria.budget}/month</span>
            </div>
          )}
          {searchCriteria?.max_commute_time && (
            <div className={styles.criteriaItem}>
              <span className={styles.criteriaIcon}>🚇</span>
              <span className={styles.criteriaLabel}>Max Commute:</span>
              <span className={styles.criteriaValue}>{searchCriteria.max_commute_time} min</span>
            </div>
          )}
          {searchCriteria?.amenities && searchCriteria.amenities.length > 0 && (
            <div className={styles.criteriaItem}>
              <span className={styles.criteriaIcon}>🎯</span>
              <span className={styles.criteriaLabel}>Amenities:</span>
              <span className={styles.criteriaValue}>{searchCriteria.amenities.length} selected</span>
            </div>
          )}
          {searchCriteria?.city && (
            <div className={styles.criteriaItem}>
              <span className={styles.criteriaIcon}>🏙️</span>
              <span className={styles.criteriaLabel}>City:</span>
              <span className={styles.criteriaValue}>{searchCriteria.city}</span>
            </div>
          )}
          {searchCriteria?.destination_neighborhood && (
            <div className={styles.criteriaItem}>
              <span className={styles.criteriaIcon}>📍</span>
              <span className={styles.criteriaLabel}>Destination:</span>
              <span className={styles.criteriaValue}>{searchCriteria.destination_neighborhood}</span>
            </div>
          )}
        </div>
      </div>
